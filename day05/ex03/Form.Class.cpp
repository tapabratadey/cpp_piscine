...
bool Form::canExecute(const Bureaucrat &exec) const
{
    if(exec.getGrade() <= this->signGrade)
        return true;
    return false;
}
...
Form::FormNotSignedException::FormNotSignedException(Form::FormNotSignedException const &copy){ *this = copy; }
Form::FormNotSignedException &Form::FormNotSignedException::operator=(Form::FormNotSignedException const &copy)
{
	(void)copy;
	return (*this);
}
